		...
		if plr.Character and plr.Name ~= Players.LocalPlayer.Name and not COREGUI:FindFirstChild(plr.Name..'_ESP') then
			...
			for b,n in pairs (plr.Character:GetChildren()) do
				if (n:IsA("BasePart")) then
					...
					local myTeam = game:GetService("Players").LocalPlayer:GetAttribute("teamname")
                    local theirTeam = plr:GetAttribute("teamname")

                    local function getFaction(team)
	                    if not team then return nil end
	                    if string.find(team, "CI") then
		                    return "CI"
	                    elseif string.find(team, "SCP") or team == "Serpents Hand" or team == "Zombie" then
		                    return "SCP"
	                    else
		                    return "Foundation"
	                    end
                    ...
                    local myFaction = getFaction(myTeam)
                    local theirFaction = getFaction(theirTeam)

                    if myFaction and theirFaction and myFaction == theirFaction then
	                    a.FillColor = Color3.fromRGB(144, 238, 144)
                    else
	                    a.FillColor = colort[theirTeam] or Color3.fromRGB(255,255,255)
                    end
				end
			end
				...
				local function espLoop()
					if COREGUI:FindFirstChild(plr.Name..'_ESP') then
						if plr.Character and getRoot(plr.Character) and plr.Character:FindFirstChildOfClass("Humanoid") and Players.LocalPlayer.Character and getRoot(Players.LocalPlayer.Character) and Players.LocalPlayer.Character:FindFirstChildOfClass("Humanoid") then
							...
							TextLabel.Text = 'Name: '..plr.Name..' | Health: '..round(plr.Character:FindFirstChildOfClass('Humanoid').Health, 1)..' | Studs: '..pos..' | Class: '..plr:GetAttribute("teamname")
						end
					else
						teamChange:Disconnect()
						addedFunc:Disconnect()
